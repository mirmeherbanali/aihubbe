...
//         .populate("userId", "firstName lastName email")
//         .populate("category", "categoryName")
//         .sort({ [sortingFor]: sort }) // index required
...
const validateCreator = async (userId) => {
  ...
  if (admin && admin.userType === "AdminUser") return true;
...
const createTool = async (req, res) => {
  try {
    ...
    if (!toolName) return response(res, false, "Tool name required");
    if (!category) return response(res, false, "Category required");
    if (!userId) return response(res, false, "userId required");
    ...
    const count = await Category.countDocuments({ _id: { $in: category } });
    ...
    if (req.files?.screenshots) {
      screenshots = await Promise.all(
        req.files.screenshots.map((f) => uploadToS3(f, "screenshots"))
      );
    }
    ...
    const parsedFeatures =
      typeof features === "string" ? JSON.parse(features) : features || [];
    const tools = category.map((catId) => ({
      toolName,
      logo: logoUrl,
      category: [catId],
      shortDescription,
...
const updateTool = async (req, res) => {
  try {
    ...
    if (!id) return response(res, false, "Tool ID required");

    if (userId) {
      const valid = await validateCreator(userId);
      if (!valid) return response(res, false, "Permission denied");
    }
    ...
    const updated = await Tool.findById(id).lean();

    return response(res, true, "Tool updated successfully", updated);
  } catch (err) {
    return response(res, false, err.message);
  }
};
...
const getToolDetailsById = async (req, res) => {
  try {
    ...
    return response(res, true, "Tool fetched", tool);
  } catch (err) {
    return response(res, false, err.message);
  }
};
